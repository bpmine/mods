...
minetest.register_chatcommand("afk", {
	...
	minetest.kick_player(param, "bouge plus.")
	minetest.log("action", "[AFKKICK] " .. name .. " kick " .. param)
        return true, "kicked " .. param .. "!"
    end,
})
		...
		if player then
			...
			if checkNow then
				...
				if info["lastAction"] + MAX_INACTIVE_TIME < currGameTime then
					minetest.kick_player(playerName, "bouge plus.")
					minetest.log("action", "[AFKKICK] kicked player")
				end
				...
				if info["lastAction"] + MAX_INACTIVE_TIME - WARN_TIME < currGameTime then
					minetest.chat_send_player(playerName,
						minetest.colorize("#FF8C00", "vous ne bougez plus attention " ..
